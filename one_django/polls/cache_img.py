...
from one_django.settings import BASE_DIR

def get_cache_code_info():
    resule = ''
    for i in range(1 , 5):
        resule += chr(random.randint(65 , 96))
    # 获取一个Image对象，参数分别是RGB模式。宽160，高30，随机颜色
    rgb1=random.randint(100,255)
    rgb2=random.randint(100,255)
    rgb3=random.randint(100,255)
    image = Image.new('RGB', (160, 30), (rgb1 , 80 , rgb3))
    # 获取一个画笔对象，将图片对象传过去
    draw = ImageDraw.Draw(image)

    # 获取一个font字体对象参数是ttf的字体文件的目录，以及字体的大小
    print(os.path.join(os.path.join(BASE_DIR, 'polls'),'cerepf.ttf'))
    font = ImageFont.truetype(os.path.join(os.path.join(BASE_DIR, 'polls'),'cerepf.ttf'),size=30)

    # 在图片上写东西,参数是：定位，字符串，颜色，字体
    draw.text((20, 0), resule, (rgb2,300,300),font=font)
